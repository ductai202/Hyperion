...
public class HashCommands
{
    ...
    {
        if (args.Length < 3 || args.Length % 2 != 1) return RespEncoder.Encode(new Exception("ERR wrong number of arguments for 'hset' command"));
        string key = args[0];
    ...
    {
        if (args.Length != 2) return RespEncoder.Encode(new Exception("ERR wrong number of arguments for 'hget' command"));
        string key = args[0];
        string field = args[1];
    ...
    {
        if (args.Length < 2) return RespEncoder.Encode(new Exception("ERR wrong number of arguments for 'hdel' command"));
        string key = args[0];
    ...
    {
        if (args.Length != 1) return RespEncoder.Encode(new Exception("ERR wrong number of arguments for 'hgetall' command"));
        string key = args[0];
